# Only add ?__a=1 in the link of the ig 
...
for doc in json_files:
    ...
    with open(doc, "r", encoding='utf-8') as json_file:
        try:
            ...
            # extra to understand better
            
            # something = content["seo_category_infos"]
            # print(something)
            # print(f"0 position {something[0]}")
            # print(f"0 1 position {something[0][1]}")

            # end extra
            user_content = content["graphql"]["user"] # Here we have al the information that we need
            username = user_content["username"] # in the content search the user name 
